...
var FinalWidget = {
  render: function (ctx) {
    ...
    if (win && state.finalStats) {
      h.push('<div class="stats"><span>姿态 ' + state.finalStats.axis + '</span><span>起落架 ' + state.finalStats.gear + '/4</span><span>襟翼 ' + state.finalStats.flaps + '/' + ctx.logic.CONFIG.FLAP_COUNT + '</span><span>剩余 ☕ ' + state.finalStats.coffee + '</span></div>');
    }

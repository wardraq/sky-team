...
**普通轮引擎**：蓝标记夹在整数之间（初始 **4 与 5 之间**，`BLUE_START=5` 是比较阈值）。引擎和 **&lt; 5** 悬停；**≥ 5 且 ≤ 橙** 前进 1；**&gt; 橙** 前进 2。因此开局和 **= 5** 前进 1 格。  
**着陆轮引擎**：第二颗引擎放上时与 **刹车限速** 比较，须 **引擎和 < 限速**。红标记槽位与限速：2 左 → **&lt;2**；2–3 间 → **&lt;3**；4–5 间 → **&lt;5**；6 后 → **&lt;7**（每激活一档刹车右移一格）。
